...
const tableComponent = () => {
  return (
    <table className="table">
      ...
      <tbody>
        {Object.keys(levelStatus).map((data) => (
          <tr key={data}>
            <td>{data}</td>
            <td>{levelStatus[data].all}</td>
            <td>{levelStatus[data].clear}</td>
            <td>{levelStatus[data].fc}</td>
            <td>{levelStatus[data].ap}</td>
          </tr>
        ))}
      </tbody>
    </table>
...
const headerComponent = () => {
  return (
    <div className="container text-center mt-5 mb-5">
      <div className="h1 mb-5">프로세카 기록체크표</div>

      <div id="description" className="h5 mb-5">
        이 체크표는 서열표가 아닙니다.
        <br></br>
        <br></br>각 그룹곡들은 난이도순으로 정렬되어있습니다.<br></br>
        ...
        <br></br>
        <br></br>이 체크표는 pc환경에서의 사용을 권장합니다. <br></br>
        크롬 브라우저 이외의 브라우저 사용 시 원활한 사용이 보장되지 않습니다.
        <br></br>
        <br></br>
        이미지와 닉네임을 등록하고 맨 아래쪽에 있는 버튼을 누르면<br></br>
        풀콤체크표.png라는 이름으로 브라우저에 지정된 다운로드 위치에
        저장됩니다.
        <br></br>
        <br></br>
        기타 피드백 및 버그사항은{" "}
        <a href="mailto:ironhiro@kakao.com?subject=[버그/피드백]">
          ironhiro@kakao.com
        </a>
        으로 보내주시길 바랍니다.
      </div>
  ...
  return (
    <div className="h5 input-group">
      <input
        ...
        aria-label="username"
      ></input>
      <button className="btn btn-primary" onClick={submitNickname}>
        등록하기
      </button>
    </div>
  );
...
const nickNameComponent = (data) => {
  return (
    <div className="h1 text-center">
      {data}
      <br></br>
      <button className="btn btn-primary" id="button_undo" onClick={undo}>
        다시 작성하기
      </button>
    </div>
  );
...
const Sections = forwardRef((props, ref) => {
  ...
  return (
    <div className="App-section container" ref={ref}>
      ...
      <div className="container">
        <div className="d-flex flex-wrap justify-content-around mt-5">
          ...
          <div className="d-flex flex-column h4 my-auto">
            <div
              className="d-flex flex-row flex-wrap h4 my-auto justify-content-center"
              style={{ fontSize: "15pt" }}
            >
            ...
            <div className="d-flex flex-wrap flex-row h4 my-auto justify-content-center">
              <div id="current_date" className="p-2">
                {getCurrentDate()}
              </div>
              <div id="blank_status" className="p-2">
                <img
                  src={process.env.PUBLIC_URL + "/assets/status/blank.png"}
                  width="25px"
                  height="25px"
                ></img>
                {chartStatus["blank"]}
              ...
              <div id="clear_status" className="p-2">
                <img
                  src={process.env.PUBLIC_URL + "/assets/status/clear.png"}
                  width="25px"
                  height="25px"
                ></img>
                {chartStatus["clear"]}
              ...
              <div id="fc_status" className="p-2">
                <img
                  src={process.env.PUBLIC_URL + "/assets/status/fc.png"}
                  width="25px"
                  height="25px"
                ></img>
                {chartStatus["fc"]}
              ...
              <div id="ap_status" className="p-2">
                <img
                  src={process.env.PUBLIC_URL + "/assets/status/ap.png"}
                  width="25px"
                  height="25px"
                ></img>
                {chartStatus["ap"]}
      ...
      <div id="chartsList">{props.props}</div>
      <div className="container mt-5">
        <p className="text-end">
          Powered by <a href="https://twitter.com/ironhiro1">@ironhiro1</a>,{" "}
          <a href="https://twitter.com/Biiiiiiiiii_0w0">@Biiiiiiiiii_0w0</a>
        </p>
      </div>
    </div>
  );
...
function uploadProfileImage(e) {
  document.getElementById("profile_img").src = window.URL.createObjectURL(
    document.getElementById("upload_file").files[0]
  );
}
...
function submitNickname() {
  let parentDiv = document.getElementById("custom-profile");
  ReactDOM.render(
    nickNameComponent(document.getElementById("input-username").value),
    parentDiv
  );
  isChecked = true;
}
...
function getRadioValue() {
  let counts = dataList.length;
  selectedDiff = document.querySelector(
  ...
  dataList.map(function (data, index) {
    switch (selectedDiff) {
      case "easy":
        if (levelStatus[data.difficulties.easy] === undefined) {
          const levelInfo = {
            all: 1,
            clear: 0,
            fc: 0,
          ...
          levelStatus[data.difficulties.easy] = levelInfo;
        } else {
          levelStatus[data.difficulties.easy]["all"] += 1;
        }
        break;
      case "normal":
        if (levelStatus[data.difficulties.normal] === undefined) {
          const levelInfo = {
            all: 1,
            clear: 0,
            fc: 0,
          ...
          levelStatus[data.difficulties.normal] = levelInfo;
        } else {
          levelStatus[data.difficulties.normal]["all"] += 1;
        }
        break;
      case "hard":
        if (levelStatus[data.difficulties.hard] === undefined) {
          const levelInfo = {
            all: 1,
            clear: 0,
            fc: 0,
          ...
          levelStatus[data.difficulties.hard] = levelInfo;
        } else {
          levelStatus[data.difficulties.hard]["all"] += 1;
        }
        break;
      case "expert":
        if (levelStatus[data.difficulties.expert] === undefined) {
          const levelInfo = {
            all: 1,
            clear: 0,
            fc: 0,
          ...
          levelStatus[data.difficulties.expert] = levelInfo;
        } else {
          levelStatus[data.difficulties.expert]["all"] += 1;
        }
        break;
      case "master":
        if (levelStatus[data.difficulties.master] === undefined) {
          const levelInfo = {
            all: 1,
            clear: 0,
            fc: 0,
          ...
          levelStatus[data.difficulties.master] = levelInfo;
        } else {
          levelStatus[data.difficulties.master]["all"] += 1;
        }
        break;
    }
  });

  const tests = category.map(function (element, idx) {
    const a = dataList.filter((data) => data.category === element);

    const b = a.slice(0).sort(function (a, b) {
      return sortByDifficulties(a, b, selectedDiff);
    });
    const c = b.map((data) => components(data));
    const d = `${process.env.PUBLIC_URL}/assets/groupunits/${categoryBg(
      element
    )}`;
    const groups_styles = {
      ...
      }/assets/backgrounds/${categoryBg(element)})`,
    ...
    const div_col1 = (
      <div
        style={groups_styles}
        className="col music-category mx-auto my-auto text-center"
      >
        <img className="groups" src={d}></img>
      </div>
    );
    const div_col2 = (
      <div style={charts_styles} className="col-9 mx-auto my-auto w-100">
        <ul className="groups_border text-center">{c}</ul>
      </div>
    );
    const row_styles = {
  ...
  ReactDOM.render(tests, parentDiv);
  let tableDiv = document.getElementById("chart_status");
  ReactDOM.render(tableComponent(), tableDiv);
}

function reload() {
  document.getElementById("blank_status").childNodes[1].nodeValue =
    chartStatus["blank"];
  document.getElementById("ap_status").childNodes[1].nodeValue =
    chartStatus["ap"];
  document.getElementById("clear_status").childNodes[1].nodeValue =
    chartStatus["clear"];
  document.getElementById("fc_status").childNodes[1].nodeValue =
    chartStatus["fc"];
}
...
function checkStatus(data) {
  const level = dataList.filter((x) => x.id == data);
  ...
  switch (clickCount[data] % 4) {
    case 1:
      ...
      switch (selectedDiff) {
        case "master":
          levelStatus[level[0].difficulties.master]["clear"] += 1;
          break;
        case "expert":
          levelStatus[level[0].difficulties.expert]["clear"] += 1;
          break;
        case "hard":
          levelStatus[level[0].difficulties.hard]["clear"] += 1;
          break;
        case "normal":
          levelStatus[level[0].difficulties.normal]["clear"] += 1;
          break;
        case "easy":
          levelStatus[level[0].difficulties.easy]["clear"] += 1;
          break;
        default:
          levelStatus[level[0].difficulties.master]["clear"] += 1;
          break;
    ...
    case 2:
      ...
      switch (selectedDiff) {
        case "master":
          levelStatus[level[0].difficulties.master]["fc"] += 1;
          break;
        case "expert":
          levelStatus[level[0].difficulties.expert]["fc"] += 1;
          break;
        case "hard":
          levelStatus[level[0].difficulties.hard]["fc"] += 1;
          break;
        case "normal":
          levelStatus[level[0].difficulties.normal]["fc"] += 1;
          break;
        case "easy":
          levelStatus[level[0].difficulties.easy]["fc"] += 1;
          break;
        default:
          levelStatus[level[0].difficulties.master]["fc"] += 1;
          break;
    ...
    case 3:
      ...
      switch (selectedDiff) {
        case "master":
          levelStatus[level[0].difficulties.master]["ap"] += 1;
          break;
        case "expert":
          levelStatus[level[0].difficulties.expert]["ap"] += 1;
          break;
        case "hard":
          levelStatus[level[0].difficulties.hard]["ap"] += 1;
          break;
        case "normal":
          levelStatus[level[0].difficulties.normal]["ap"] += 1;
          break;
        case "easy":
          levelStatus[level[0].difficulties.easy]["ap"] += 1;
          break;
        default:
          levelStatus[level[0].difficulties.master]["ap"] += 1;
          break;
...
function components(data) {
  ...
  /*
        levelStatus => level: {clear: 0, fc: 0, ap: 0, all: 레벨 개수}
    */
  ...
  const tooltipMessage = `Title: ${data.title}
        Lv
  ...
  return (
    <li className="jacketInfo" style={listStyle}>
      <canvas
        id={canvasName}
        onClick={drawDash}
        data-for={"chart" + data.id.toString()}
        data-tip
        className="jackets"
        style={imageStyle}
      ></canvas>
  ...
  const tested = await Promise.all(
    category.map(function (element, idx) {
      return getValues(element)
        ...
        .then(function (data) {
          const b = `${process.env.PUBLIC_URL}/assets/groupunits/${categoryBg(
            element
          )}`;
          const group_name =
            categoryBg(element).replace(".png", "") == "other"
              ? categoryBg(element).replace(".png", "")
              : "groups";
          ...
          const div_col2 = (
            <div style={charts_styles} className="col-9 mx-auto my-auto w-100">
              <ul className="groups_border text-center">{data}</ul>
            </div>
          );
          const row_styles = {
            border: "10px solid " + borderColor[element],
